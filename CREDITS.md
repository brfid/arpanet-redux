...
- *Gap:* the raw historical distribution tape reproduced in [`docs/research/imp11a-device.md`](docs/research/imp11a-device.md) (the source `extract-v6-tape.py` reads) was preserved and made available by a named individual, believed to be Ken Wellsch, but the exact citation for that preservation credit is temporarily lost — it isn't recorded anywhere in this repository's history or docs. Track down and confirm the actual attribution (TUHS archive metadata or mailing-list history is the likely place to start), then replace this note with a proper credit line.
