...
The NCC results can be watched through passive local browser displays that keep evidence authority visible. The historical-line desk preserves configured-only links, direct endpoint reports, in-memory reconciliation, and completed-summary authority as separate layers; the message-journey bench preserves configured crossings, direct H316 observations, harness-derived peer delivery, and reducer assessments across the Network UNIX-to-ITS route. The integrated composition currently produces both inputs and a separate composition verdict; combining them into one passive phase-aware desk is the next bounded NCC slice. See the [historical display](docs/runbook.md#view-a-growing-ncc-historical-event-sidecar) and [journey display](docs/runbook.md#view-a-growing-typed-message-journey) commands in the existing-laboratory runbook.
