...
The passive NCC board shows configured structure and direct report activity while a result grows. After the accepted failover artifacts validate, it shows the acknowledged direct-cable cut and the typed alternate route through IMP 7 without promoting candidate report-line numbers. `make ncc-failover` runs the existing formal scenario beside the board; `make view-ncc-failover` replays the retained canonical result without launching a simulator. The browser remains read-only.
