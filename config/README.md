...
[`topologies/ncc-alternate-path-fault.json`](topologies/ncc-alternate-path-fault.json) reuses that exact NCC host attachment and direct IMP 5/6 report-line identity, then adds IMP 7 as an alternate route. Only the direct binding carries report-line fields: no line number is inferred for either alternate binding. Its ten environment variables represent the NCC host cable, two alternate modem cables, both simulator ends of the direct cable, and the two ports held by the direct-line relay. The relay initially forwards the direct cable in both directions, then remains bound while dropping both directions so a post-cut report from IMP 6 can reach the receiver only through IMP 7 and IMP 5. This is a deliberately nonhistorical fault-evidence composition, not a claim about a historical ARPANET route.

`make smoke-ncc-alternate-path` owns this composition's verification and lifecycle. The [test plan](../docs/test-plan.md) defines its acceptance checks, the [runbook](../docs/runbook.md) gives the invocation, and the [dated experiment](../docs/experiments/2026-08-31-ncc-alternate-path-fault.md) records why the composition and the neighborless-down reducer rule exist.
