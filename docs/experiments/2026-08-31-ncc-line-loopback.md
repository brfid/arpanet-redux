...
## Read-only reconciliation after ADR-011

[ADR-011](../adr/0011-ncc-looped-report-self-neighbor.md) accepts only the exact self-neighbor/looped signature exposed above. The existing external analysis helper re-read the original topology, relay record, receiver result, and historical-event sidecar from `ncc-line-loopback-experiment-20260831T232858Z`, writing its new derived output outside the immutable result directory. All eight analysis checks passed. The same pre-loop support sequences 80 and 90 reduce to `up`, and the same final support sequences 397 and 419 reduce to `looped`.

The derived verdict SHA-256 is `fb7a0fb128f6a62b5f90755e25f1518dcea55e409fca26df0498e858f196c948`. A before-and-after checksum comparison covered every file in the original result directory and found no change. The exact run's manifest and original `verdict.json` remain failed; this is a new interpretation under a tested rule, not a relabelled run. Focused tests additionally prove that configured-peer, absent-neighbor, and third-party-neighbor looped reports remain contradictory, as do self-neighbor up and down reports.

## Promoted canonical gate

The proved mechanism was promoted without importing any external artifact: an owned byte-preserving reflector, a structured evaluator, a bounded smoke runner, focused tests, and the matching test-plan/runbook contract. Commit `dacd63d8afe080beef14ed1b7497f27ebae9396c` is the clean repository identity recorded by the canonical run. It retained clean ARPANET-in-a-Box source `78123c77b20dadd9b5967b184dbcb4195185eea6`, clean H316 SIMH source `feb155fbc49333e879ab082d481e6dcce27d2d91`, H316 executable SHA-256 `bdbcdffc63ada17c9ec6c7151aba42fd96388ff33d41ec6d42c5b27f47cfb994`, firmware SHA-256 `bc4870059b9131636a49dec53399b8f654ba5c146bd09c32d48ab65d5309c771`, and base-configuration SHA-256 `b3c4fe408c3ec2515f629eea327f4e2f33f692997d4c45e423853da6d1800d78`.

The committed topology SHA-256 remained `0132a4350cea8b93624b18573bacc56302d922b1f896dda4c2e83403c670f806`; the IMP 5, IMP 6, and IMP 7 command-file SHA-256 values remained `eb57f423646f1d7a98b42d46f015348cb7c1fa10528ead40a9621b6b91c00855`, `923d9a6dceb36f4adb31c72120582f676639927510237ecb57a6c2dc6a319c44`, and `726e96786b81dd46a7ae6c253af4439ee7acfc01c3c05393bc9cc6b1bb400f51`. The committed runner, reflector, evaluator, and receiver SHA-256 values were `ce62c6f6a1f946860e3c5e4c75a37226b642b6cf7c973cc1e0ccf286d2bef146`, `1f0082dfa4f0a113a4fb154223a22d4ad76f2d7363be4591048b373446bd4f45`, `1fc9f903ba1fefbd4868a13c29df84fc8233193020abeef60e150615c15abb2c`, and `1707c9046bb19e927fca5f69033337179e5b7d5c7f48b37a9c50995d2648d89e`.

Exact run `ncc-line-loopback-canonical-20260831T235244Z` used the committed entry point:

```sh
make LAB_ROOT=/Users/brf/src/arpanet-redux-lab RUN_ID=canonical-20260831T235244Z smoke-ncc-line-loopback
```

The reflector began returning each direct datagram to its source at `2026-08-31T23:53:37.741940Z`. Before that boundary it forwarded 540 datagrams from IMP 5 and 535 from IMP 6; afterward it reflected 1,260 and 1,264 datagrams respectively, with no unexpected source. The evaluator reconciled the last pre-loop direct observations as `up` from sequences 80 and 90. IMP 5 first reported line 1 looped-to-self at sequence 199 and `2026-08-31T23:54:00.116789Z`; IMP 6 followed at sequence 210 and `2026-08-31T23:54:00.128427Z`. Each endpoint produced seven looped-to-self observations, and their final pair was `looped`, supported by sequences 397 and 419.

The receiver recorded 77 complete messages and 424 direct events. Its checksum-valid stream contained 14 IMP 5, 13 IMP 6, and 12 IMP 7 trouble reports plus 12 IMP 5, 11 IMP 6, and 11 IMP 7 throughput reports. Ten trouble reports from each of IMPs 5 and 6 arrived after reflection began. All eight structured evaluator checks passed. The manifest records `repository.tracked_dirty=0`, both source trees clean, both owned controller exit statuses zero, `cleanup.completed=1`, `outcome=passed`, and `exit_status=0`; both controller error logs are empty. A separate post-run audit found none of the five recorded PIDs, ten sockets, or cooperative port locks alive.

This passing canonical run does not rewrite or supersede the intentionally failed exploratory verdict. Together the records retain the experimental progression: the forwarding control established the healthy baseline, the first reflection run exposed the reducer mismatch, ADR-011 narrowed the rule, the immutable sidecar proved the new interpretation, and the promoted clean run exercised that rule and the complete supported lifecycle.

## Limits

The experiment proves one bounded project composition, one two-ended reflection method, repeated reciprocal firmware loop indications, continued alternate-path report delivery, and one supported clean lifecycle. It does not establish that the transient down-to-self form has independent durable meaning, identify IMP 7 or any endpoint as a historical site, infer loopback from silence or topology alone, establish a universal simulator-device-to-report-line mapping, or authorize persisted reducer output. The exploratory loop manifest remains `outcome=failed` because its exact contemporary analyzer correctly exposed the then-unresolved reducer mismatch; it must not be relabelled after the fact.
