...
Python controller-side ownership is split by responsibility across ordinary `ncc` modules. `ncc.harness_process` owns PTY and IMP processes plus aggregate shutdown; `ncc.harness_manifest` owns validated manifest append and streamed artifact hashing; `ncc.harness_config` owns the shared UDP-port environment contract and host-106 attach-only configuration; and `ncc.harness_imp` owns recovered-IMP log readiness, watchdog interpretation, and modem-message correlation. The two-ITS controller imports these owners directly; dependent controllers temporarily retain their existing aliases through that controller until their own migrations are separately verified.
