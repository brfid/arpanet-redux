...
| Historical terminal stream | Records bounded directional bytes and local safety decisions for a character-oriented Network UNIX console; version 1 names one direct route and version 2 names the controller-owned direct-to-alternate transition without claiming command/result grammar |
...
- a human-operated extension that accepts one local direct-link cut only after a proved transaction and requires continued service in the same guest connection over the typed IMP 7 route;
...
The browser cannot send TELNET input, switch a link, signal or restart a component, load memory, transfer core, run DDT, or mutate a result. Each formal or interactive failover controller alone owns its run-local cut request; the interactive controller also owns the foreground terminal and reserves Control-^ only in that two-route profile. The direct character bridge continues to forward Control-^, blocks the configured SIMH WRU byte, and never exposes `sim>`.
...
The terminal-side seams are accepted. The prompt-framed mode retains deterministic command/result evidence. The direct character-oriented mode starts on Network UNIX, delegates connection, option, mode, and protocol behavior to the preserved guest client, retains exact directional bytes, blocks simulator WRU, safely projects output, and closes an application claim only when direct guest, ITS, IMP, and cleanup evidence exists. Its failover profile adds one controller-local cut after a proved pre-cut transaction and requires a structured post-cut response in the same guest connection plus the already typed alternate journey. It starts no passive receiver and makes no report-line claim. None of the terminal modes adds a browser endpoint, ingress parser, guest protocol implementation, or simulator authority; the direct Gate 4H host parsers remain a separate bounded controller path. Browser input remains a separate authority decision. Candidate mapping promotion, browser-side faults, per-component restart, new hosts or IMPs, other host-interface seams, and original NCC compatibility also remain separate decisions. See [workstreams](workstreams.md) for current boundaries.
