...
The historical-line pair now has genuine complete `up`, `down`, and raw reciprocal `looped` evidence. Exact run `ncc-imp6-original-20260831T215714Z` supplies the independently attributed reciprocal line-1 `up` pair; exact run `ncc-alternate-path-fault-canonical-20260831T230203Z` keeps both reporting IMPs observable through IMP 7, proves bidirectional direct-cable drops, and supplies the reciprocal `down` pair. External-laboratory run `ncc-line-loopback-experiment-20260831T232858Z` preserved that alternate report route while byte-exact reflection caused each endpoint to report `looped` with itself, rather than the configured remote peer, as neighbor. Its immutable verdict is intentionally failed because the current reducer calls that genuine pair contradictory. The next bounded historical-line task is a narrow source-only decision and focused tests for the self-neighbor loop signature, followed by read-only reevaluation of the preserved sidecar; only if that passes should the owned reflection harness be promoted into a supported gate. This remains distinct from the separate product decision about persisting reducer output or bridging it into the accepted completed-run/controller-live contracts; that bridge still requires explicit versioning and compatibility work. IMPs 5, 6, and 7 remain configured test components, not asserted historical sites.
