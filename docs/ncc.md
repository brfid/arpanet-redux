...
The implemented slice remains deliberately bounded: `ncc/` decodes the recovered 1973 IMP firmware's original Type 301 and patched Type 303 trouble-report forms into topology-neutral events, reconciles synthetic paired historical line endpoints against a typed nominal topology, validates a derived completed-run summary, adapts formal two-ITS result artifacts, renders deterministic local replay and a static viewer, and publishes the formal controller's existing lifecycle observations as a bounded JSON Lines stream. A passive snapshot reader marks expired observations stale without moving or discarding nominal topology. The attachment seam includes a passive host-interface receiver, an old-style leader adapter, a shared IMP 5/IMP 6 topology input, and a separate validated historical-event sidecar for direct report recording/replay. It still has no normalized live-report publisher or browser-based live display.
...
5. Add nominal-topology reconciliation, paired line state, report timeouts, recording, and replay for genuine IMP reports. **The source-only paired-line and report-timeout reducer is implemented.** [`ncc.historical_events`](../ncc/historical_events.py) now provides a separately versioned, validated direct-event record and replay input for actual reports; it does not yet persist reducer output or adapt it to the accepted run summary/live stream. [ADR-007](adr/0007-ncc-historical-event-stream.md) and the [bounded record experiment](experiments/2026-08-31-ncc-historical-event-record.md) own that boundary and its exact-run evidence.
...
The next engineering task is to establish a primary format description for the 1973 Type 302 throughput body before decoding it or broadening the direct-event vocabulary. Any extension must preserve source attribution, observation time, wire code, and the distinction between historical reports, modern harness observations, and derived conclusions. It must add no viewer process control or two-ITS controller configuration. The local IMP 6 proof peer remains a configured test component, not an asserted historical route or reporting source.
