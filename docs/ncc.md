...
The next message-journey action is for a future heterogeneous or network-expansion harness to emit the typed direct observations at its exact shared-topology route boundaries, beginning with established H316 transfers and adding KA10 or IMP11-A parsers only when their complete extraction formats are proven. Any durable journey result or bridge to the accepted completed-run/controller-live contracts still requires an explicit versioning and compatibility decision.

The separate historical-line task remains to define explicit report-line identities in the shared topology and obtain a separately attributed IMP 6 endpoint report before feeding that pair to the topology-aware reducer. A one-way IMP 5 report is not evidence of a complete line state. [ADR-009](adr/0009-ncc-paired-line-topology-boundary.md) records this boundary. Any later durable reducer output must not adapt itself to the accepted two-ITS result contract or controller-owned live stream by implication. The work must preserve source attribution, observation time, wire code, and the distinction between historical reports, modern harness observations, and derived conclusions. It must add no viewer process control or two-ITS controller configuration. The local IMP 6 proof peer remains a configured test component, not an asserted historical route or reporting source.
