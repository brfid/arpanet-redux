...
The implemented slice remains deliberately bounded: `ncc/` decodes the recovered 1973 IMP firmware's original Type 301 and patched Type 303 trouble-report forms into topology-neutral events, reconciles synthetic paired historical line endpoints against a typed nominal topology, validates a derived completed-run summary, adapts formal two-ITS result artifacts, renders deterministic local replay and a static viewer, and publishes the formal controller's existing lifecycle observations as a bounded JSON Lines stream. A passive snapshot reader marks expired observations stale without moving or discarding nominal topology. The attachment seam now includes a passive host-interface receiver, an old-style leader adapter, and a shared IMP 5/IMP 6 topology input that prove transport ingress and attributed report decoding without simulator control. It still has no actual-report recorder, normalized live-report publisher, or browser-based live display.
...
3. **Historical NCC telemetry:** attach a project-authored receiver at host 0 of BBN IMP 5 and feed genuine IMP status and throughput reports into the event stream. The ingress adapter decodes the original Type 301 and patched Type 303 trouble-report wire forms, attributes them from the old-style leader, and preserves the on-wire code; Type 302 throughput is not yet decoded.
...
5. Add nominal-topology reconciliation, paired line state, report timeouts, recording, and replay for genuine IMP reports. **The source-only paired-line and report-timeout reducer is implemented; recording and replay of actual reports remain.**
6. Attach the receiver at BBN IMP 5 after the current topology work is integrated and the required host-interface behavior is proven in isolation. **The passive transport proof, shared topology composition, old-style leader adapter, and attributed trouble-report decode are implemented:** the receiver has sent host readiness, received IMP readiness, reassembled complete IMP output through a live adjacent IMP peer, and emitted direct events from genuine patched Type 303 reports attributed to IMP 5. The [transport experiment](experiments/2026-08-30-ncc-host-interface-proof.md) and [report-ingress experiment](experiments/2026-08-31-ncc-report-ingress.md) record the direct derived results and their limits.
...
The next engineering task is to add a bounded recorder and replay input for genuine passive trouble-report events, after defining their relationship to the accepted run-summary and live-stream contracts. It must preserve the original Type 301 versus patched Type 303 wire code, source attribution, observation time, and the distinction between historical reports, modern harness observations, and derived conclusions. It must add no viewer process control or two-ITS controller configuration. The local IMP 6 proof peer remains a configured test component, not an asserted historical route or reporting source.
