...
The historical-line pair now has genuine complete `up`, `down`, and reciprocal `looped` evidence. Exact run `ncc-imp6-original-20260831T215714Z` supplies the independently attributed reciprocal line-1 `up` pair; exact run `ncc-alternate-path-fault-canonical-20260831T230203Z` keeps both reporting IMPs observable through IMP 7, proves bidirectional direct-cable drops, and supplies the reciprocal `down` pair; exact run `ncc-line-loopback-canonical-20260831T235244Z` retains that alternate observation route, proves bidirectional byte-exact reflection, and supplies the reciprocal self-neighbor `looped` pair. The supported historical-line state-coverage slice is complete. No further evidence-producing line task is selected: the next change on this track would be the separate product decision about persisting reducer output or bridging historical reports into the accepted completed-run/controller-live contracts, and that requires an explicit versioning and compatibility discussion first. IMPs 5, 6, and 7 remain configured test components, not asserted historical sites.
