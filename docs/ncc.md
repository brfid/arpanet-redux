...
The implemented slice remains deliberately bounded: `ncc/` decodes the recovered 1973 IMP firmware's Type 301 trouble report into topology-neutral events, reconciles synthetic paired historical line endpoints against a typed nominal topology, validates a derived completed-run summary, adapts formal two-ITS result artifacts, renders deterministic local replay and a static viewer, and publishes the formal controller's existing lifecycle observations as a bounded JSON Lines stream. A passive snapshot reader marks expired observations stale without moving or discarding nominal topology. The next attachment seam is also implemented: a passive host-interface receiver and a shared IMP 5/IMP 6 topology input prove transport ingress without simulator control. It still has no host-leader adapter, actual-report recorder, normalized live-report publisher, or browser-based live display.
...
6. Attach the receiver at BBN IMP 5 after the current topology work is integrated and the required host-interface behavior is proven in isolation. **The passive transport proof and shared topology composition are implemented:** the receiver has sent host readiness, received IMP readiness, and reassembled complete opaque IMP output through a live adjacent IMP peer. The [bounded experiment](experiments/2026-08-30-ncc-host-interface-proof.md) records the direct derived result and its limits.
...
The next engineering task is to adapt a completed passive host-interface message into the versioned report decoder only after the selected 1822 leader interpretation is supported by primary documentation and exact-run evidence. It must emit direct Type 301 observations into the accepted boundaries, preserve the distinction between historical reports, modern harness observations, and derived conclusions, and add no viewer process control or two-ITS controller configuration. The local IMP 6 proof peer remains a configured test component, not an asserted historical route or reporting source.
