...
**Observed through 2026-08-31:** SRI/NOSC Network UNIX V6 is now a proven formal heterogeneous endpoint. Published IMP11-A revision `2722eef44f68642eaab9f5d4e989ccd26e55e7de` preserves returned frames across the guest's leader and continuation DMA buffers, allowing the already-correct ITS STR/RTS/STR control stream to reach the daemon. Exact formal run `pdp11-its-telnet-20260831T200436Z` verifies the receipt-bound guest build, records `Connection open`, ITS service job `53TLNT`, `TTY 53`, the welcome banner, a remote `:TIME` response with time, date, and uptime, correlated post-probe traffic through both IMPs, and complete cleanup. [The IMP11-A device record](imp11a-device.md#formal-gate-4h-promotion-2026-08-31) retains the detailed evidence; [workstreams](../workstreams.md) owns current handoff state.
...
The experiment diverged from its original mechanics without changing its objective: the filesystem injector placed the existing kernel and daemon directly, the client was built in-guest, and the preserved NCP ran through the existing two-IMP fabric. The formal PDP-11 console contains both a command and its structured ITS response, while the two IMP logs corroborate the path after captured probe offsets, so step 6 and Gate 4H are complete. The detailed record of the byte-order, RFNM-accounting, receive-continuation, build-receipt, and formal-run evidence is in [the IMP11-A record](imp11a-device.md); [workstreams](../workstreams.md) records optional follow-up rather than an open application boundary.
