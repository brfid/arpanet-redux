...
**Observed through 2026-08-31:** The IMP11-A device, bootable `green/unix` system, live daemon, two-IMP attachment, and guest-built TELNET client were demonstrated. Published adapter corrections now serialize PDP-11 output high-bit first and present returned words in guest byte order, so separate RST and RFC messages cross both IMPs, ITS accepts the complete NOSC header, and returned host-106 traffic reaches the daemon. A fresh staged-daemon trace also closes the later `Qenter` contradiction: the project's pre-adapter-fix `rfnm_bm` clear double-counted the retained RST, while restoring the preserved daemon's original RST → RFNM → RFC → RFNM sequence balances `h_pb_sent` and `h_pb_q` exactly. TELNET still times out without an ITS service job or usable session; the next boundary is returned host-host control processing and socket matching after the balanced RFC. [The IMP11-A device record](imp11a-device.md) retains the detailed evidence; [workstreams](../workstreams.md) owns the active task.
...
The experiment diverged from its original mechanics without changing its objective: the filesystem injector placed the existing kernel and daemon directly, the client was built in-guest, and the two-IMP network half completed. The detailed record of those deviations, the fixed output-ordering defect, ITS's confirmed acceptance, and the remaining input DMA presentation mismatch is in [the IMP11-A record](imp11a-device.md); the active next action is in [workstreams](../workstreams.md).
