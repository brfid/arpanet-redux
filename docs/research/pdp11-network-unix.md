...
**Observed through 2026-08-30:** The IMP11-A device, bootable `green/unix` system, live daemon, two-IMP attachment, and guest-built TELNET client were demonstrated. The connection to ITS host 106 does not complete, but the root cause is now identified and it isn't a network or device problem: the preserved NCP daemon's own defensive pre-connection RST leaves the host's RFNM-tracking bit set, which then blocks the real RFC it queues moments later from ever being sent — a bookkeeping deadlock inside the 1970s daemon source itself, confirmed by both source tracing and an empirical rerun, leaving the guest-visible application proof open pending a decision on whether to patch the preserved daemon. [The IMP11-A device record](imp11a-device.md) retains the detailed evidence; [workstreams](../workstreams.md) owns the active task.
