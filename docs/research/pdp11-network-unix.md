...
**Observed through 2026-08-31:** The IMP11-A device, bootable `green/unix` system, live daemon, two-IMP attachment, and guest-built TELNET client were demonstrated. The daemon's RFNM bookkeeping deadlock is fixed, and the apparent IMP 62 relay failure is also fixed: the device now converts PDP-11 little-endian DMA words to the IMP's high-bit-first wire order, so the real RFC crosses both IMPs and reaches ITS. A correlated KA10 trace proves ITS accepts the complete NOSC header and returns RRP plus subsequent NCP control traffic; the earlier apparent error-in-leader response was a misread `Data from host zero` diagnostic. TELNET still times out because the unchanged input DMA path presents each returned numeric word to little-endian guest memory with its two network bytes reversed, so the daemon discards the valid replies. [The IMP11-A device record](imp11a-device.md) retains the detailed evidence; [workstreams](../workstreams.md) owns the active task.
