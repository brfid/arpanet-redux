...
**Observed through 2026-08-31:** SRI/NOSC Network UNIX V6 is now a proven heterogeneous endpoint. Published IMP11-A revision `2722eef44f68642eaab9f5d4e989ccd26e55e7de` preserves returned frames across the guest's leader and continuation DMA buffers, allowing the already-correct ITS STR/RTS/STR control stream to reach the daemon. Exact final run `imp11a-telnet-usable-final-20260831T193000Z` records `Connection open`, ITS `TTY 53`, the welcome banner, and a remote `:TIME` response with the time, date, and uptime on the PDP-11 console, with corroborating IMP and KA10 traffic. [The IMP11-A device record](imp11a-device.md#receive-continuation-root-cause-and-usable-telnet-proof-2026-08-31) retains the detailed evidence; [workstreams](../workstreams.md) owns current handoff state.
...
The experiment diverged from its original mechanics without changing its objective: the filesystem injector placed the existing kernel and daemon directly, the client was built in-guest, and the preserved NCP ran through the existing two-IMP fabric. The final PDP-11 console contains both a command and its structured ITS response, while the IMP and KA10 logs corroborate the path, so step 6's acceptance criterion is complete. The detailed record of the byte-order, RFNM-accounting, and receive-continuation corrections is in [the IMP11-A record](imp11a-device.md); [workstreams](../workstreams.md) records optional follow-up rather than an open application boundary.
