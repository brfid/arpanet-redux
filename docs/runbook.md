...
The direct Network UNIX-to-ITS smoke prints its result path, preflight stages, and the controller's awaited conditions with their timeout budgets. The same diagnostic can inspect a growing result from a second terminal; it reports the last recorded stage without asserting current activity. A controller failure names its stage and component or missing readiness condition. During shutdown, allow the existing bounded cleanup to finish: repeating Control-C or TERM does not shorten the first termination's cleanup allowance.

The direct launcher retains missing input, busy build lease, receipt, and simulator-verification failures once it has created its new manifest. Make verification prerequisites can still fail before the launcher runs; keep that terminal output and use `make doctor` for the missing or mismatched input. An invalid invocation, an unwritable result location, or a result-name collision can likewise leave no new record. Use a fresh `RUN_ID` after correcting the cause. Never infer ownership from an older manifest's PIDs or remove a busy lease merely because a diagnostic is unfinished.
...
The last recorded checkpoint comes from recognized fields in manifest order. It describes retained harness metadata, not the last instruction executed, a current network state, or a newly validated application result. Controller cleanup and outer-runtime cleanup are reported separately; missing cleanup records stay unknown. The outer layer recognizes the shared runtime's final cleanup result, `cleanup.outer-runtime`, and the historical-line scenarios' `cleanup.completed` flag, rejecting disagreement. Older failure paths can perform cleanup without retaining a record, and this command does not invent that evidence.

New runs also retain explicit launcher failure descriptions, handled HUP/INT/TERM signals, and the final cleanup attempt's status and failed resources. A cleanup failure prevents a successful launcher exit; an existing nonzero launcher status is preserved. Missing details remain explicit, including failures before a result exists and uncatchable interruptions. See [launcher termination records](harness.md#launcher-termination-records) for the fields and limits.

When available, the report shows a bounded tail of the fixed launcher, controller, port-lease, receiver, or fault-instrument diagnostic logs. These excerpts are uninterpreted error output; helper errors may have been handled successfully, and they cannot supply a missing packet observation or gate verdict. Terminal controls are escaped in the human-readable report. Raw excerpts remain external laboratory material under [NOTICE](../NOTICE.md); inspect them before sharing diagnostic output.
