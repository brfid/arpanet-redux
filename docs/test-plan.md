...
## Gate 4K: Interactive same-session TELNET failover

Compose Gate 4J's character-oriented Network UNIX terminal with the accepted four-IMP application-failover topology and its direct-link relay. One foreground controller owns operator input, every simulator PTY, the run-local cut request, retained terminal stream, route evidence, verdict, and cleanup. Apply Gate 4J's source, executable, build-receipt, readiness, preserved-client, connection, TELSER, terminal-safety, directional-byte, fatal-condition, and cleanup requirements plus the accepted failover gate's relay, route, and journey requirements. Accept the interactive failover extension only if:

1. The version-2 session-start record binds the exact run and repository revision, the initial host-176-to-host-106 direct route, the post-cut route through IMP 7, failover-controller ownership, operator and PDP-11 byte sources, the unchanged seven-bit terminal profile, local Control-] exit, blocked SIMH WRU, local Control-^ application-link cut, and finite directional and chunk limits. The strict reader validates every record, cumulative count, and digest and rejects version-1 failover controls or version-2 route and ownership mismatches.
2. The operator invokes the preserved no-argument `/usr/bin/telnet`, enters `connect - -h 106`, observes exactly one `Connection open`, receives the ordered ITS greeting, and completes one structured `:TIME` response before requesting the cut. ITS records the matching TELSER job, and exact significant direct-route content correlates through IMPs 62 and 6 after the controller's fixed offsets.
3. Control-^ is intercepted locally and never reaches the guest. Before the pre-cut requirements pass, it records `application-link-cut-not-ready` and leaves the relay intact. The one accepted request records `application-link-cut-requested`, exclusively creates the run-owned request file, and must match the relay's atomic cut acknowledgement and fault timestamp; another request records `application-link-cut-already-requested` and cannot create a second fault.
4. Before accepting more application evidence, the controller observes the direct modem devices dead at IMPs 62 and 6, both alternate endpoint devices ready, both IMP 7 route devices ready, the accepted post-cut settle interval, and live host processes. The relay records positive forwarded counts in both directions before the cut, positive dropped counts in both directions after it, and no unexpected source.
5. Without another `Connection open`, one post-cut `:TIME` returns structured time, date, and uptime. Post-cut host-interface traffic is bidirectional at IMPs 62 and 6, both IMP 7 directions carry the transaction, and no session-close, alternate-device-dead, fatal transport, or process-exit condition occurs.
6. The controller writes and reads back a ten-observation direct pre-cut journey and a fourteen-observation post-cut journey on `route:host176-to-host106-alternate`. The latter retains diagnosis `missing-boundary` at `boundary:request:8`; application output cannot fill that boundary. The evaluator closes over matching topology, run, repository, terminal digest, cut, application, journey, pinned-input, outcome, and cleanup identities without requiring or inferring passive NCC reports.
7. Control-] ends the foreground session, terminal settings are restored, every owned process exits, the outer runtime cleanup passes, and a malformed, incomplete, reconnected, uncut, over-limit, interrupted, or unsupported session fails acceptance while retaining its bounded result.
...
Runtime evidence tests launch real shell children to verify explicit check failures, handled HUP/INT/TERM, uncatchable termination, cleanup retries, failed resource removal, primary exit-status preservation, manifest collisions, write failures, and bounded failure text. A cleanup-only failure must turn zero into a nonzero result; a recovered helper error must not become a terminal failure. New termination and cleanup fields must reconcile with legacy scenario records, and malformed or contradictory records must fail closed. External verification uses a narrow accepted smoke and a deliberately interrupted or failed launcher, retaining both results outside Git.
