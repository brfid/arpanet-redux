...
The historical-event sidecar must have one validated header with a shared-topology snapshot, topology/interface identities, a run start, and project-authored provenance. Every complete later line must be an ordered direct historical event attributed to a known reporting IMP and one of the supported report, host-interface, or local-line shapes. It must preserve the received report code in details, reject invalid source/subject/state combinations without appending a partial batch, tolerate only an interrupted final line, and replay direct states without inferring topology edges, timeouts, partitions, gates, or a completed-run verdict.
