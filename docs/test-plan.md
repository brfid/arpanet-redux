...
An operator may exit before starting or connecting the client; that run may pass the bounded terminal lifecycle but does not satisfy Gate 4J and must retain `connection_open=0`. This gate proves a safe seven-bit teletype interaction with the preserved guest TELNET implementation. It does not prove a particular cursor-addressed terminal type, full-screen or paged ITS programs, browser input, application-link failover during a human session, or either missing Gate 4H guest-ingress boundary. It emits no message-journey sidecar and supplies no host-ingress parser or new simulator authority.
...
Start Network UNIX 176, IMP 62, IMP 6, ITS 106, the IMP 5/6/7 NCC triangle, and the passive receiver as one composition. Preserve only the evidenced IMP 5 line 1 / IMP 6 line 1 mapping. Apply every Gate 4H requirement. Accept the additional NCC claim only if:
