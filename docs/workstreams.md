...
| `/Users/brf/src/arpanet-redux-worktrees/retention-helper` | `codex/retention-helper` | Ship and verify the retained-result media-pruning helper |
...
|---|---|---|---|
| Documentation | Current pages use one owner per concern; experiments and research remain dated records; source, link, and soft-wrap checks pass. A tracked, fail-closed retained-result media-pruning helper is selected on `codex/retention-helper` | Ship the helper with source-only tests and update its runbook procedure; do not mutate existing laboratory results while verifying it | [README](../README.md), [architecture](architecture.md), [test plan](test-plan.md) |
