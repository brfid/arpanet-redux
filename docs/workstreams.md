...
The root cause is now identified, and it is not a network, device, or leader-format problem: the guest's 1822-level leader is confirmed well-formed (matching the pinned `linux-ncp`'s own proven-working leader format field-for-field), and this project's IMP fabric is confirmed working. The blocker is inside the preserved NCP daemon (`ncpd/` in `network-unix-v6`) itself: its own defensive `chk_host()` sends a reset (RST) before every connection attempt to a host not yet known to be up, and that RST's own send marks the host's RFNM-tracking bit; the real RFC gets queued moments later but `send_pro()` refuses to send anything else to that host while the bit is set, and nothing in this environment ever clears it (that requires an IMP-level retransmission-failure notification this project's H316 transport code has no concept of). See [the dated root-cause update in the IMP11-A device record](research/imp11a-device.md#the-real-root-cause-an-rfnm-bookkeeping-deadlock-not-a-leader-or-transport-problem-2026-08-30-continued) for the full source trace and an empirical confirmation (a 150s settle, six times the original, changes nothing). The next decision is whether to patch the preserved daemon source to avoid the RST-before-RFC collision, or find another way to seed the host as "up" first — not yet made.
