...
| `/Users/brf/src/arpanet-redux-worktrees/run-failure-evidence` | `codex/run-failure-evidence` | Retained launcher failures, handled interruptions, and outer-runtime cleanup |
...
|---|---|---|---|
| Laboratory usability | First batch is published on `origin/main`: `make diagnose-run RESULT=...` explains retained successful, failed, and unfinished runs. All 359 source tests completed with one expected UDP sandbox skip; runtime checks passed with their expected socket skip. Five retained results produced deterministic reports without changing their files | Second batch selected: retain explicit launcher failure reasons, handled HUP/INT/TERM signals, and the final outer-runtime cleanup result; expose those records through the diagnostic. Preserve primary exit failures, reject cleanup-only success, and verify failure, interruption, and successful-run paths. Historical protocols, guest media, browser authority, and acceptance rules stay outside this batch; independent base-media reconstruction remains a later priority | [Harness](harness.md#retained-run-diagnostics), [run diagnostics](runbook.md#diagnose-a-retained-run), [NCC authority](ncc.md) |
