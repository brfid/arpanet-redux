...
| `/Users/brf/src/arpanet-redux-worktrees/interactive-failover` | `codex/interactive-failover` | Human-operated same-session TELNET failover over the accepted alternate route |
...
One optional branch is parked with no attached worktree: `research/historical-addressing-model`, on `origin`, currently 23 commits behind and five commits ahead of `main`. It is not queued for review or integration. See the historical addressing handoff below before deliberately reviving work that touches shared topology, address identifiers, or `config/`.
...
| PDP-11 TELNET | Gate 4H, receipt-bound media, remote `:TIME`, correlated IMP evidence, and a complete twelve-observation direct journey include independent KA10 request-ingress and IMP11-A reply-ingress traces; deterministic repeated commands, clean option negotiation, and a safe character-oriented Network UNIX terminal continue to use the preserved client's command, mode, and protocol controls | Selected: compose the character terminal with the accepted four-IMP application-failover topology. One controller-only key may cut the run-owned direct relay only after a connected pre-cut structured `:TIME`; acceptance requires an acknowledged direct-dead/alternate-ready transition and a structured post-cut `:TIME` in the same guest session with observed traffic through IMP 7. Do not add browser authority, topology, report-line promotion, a guest-protocol implementation, or a new host-interface claim | [IMP11-A reply-ingress result](experiments/2026-09-04-imp11a-reply-ingress.md), [application-failover result](experiments/2026-09-01-ncc-pdp11-its-application-failover.md), [historical terminal result](experiments/2026-09-01-historical-network-unix-telnet-terminal.md) |
