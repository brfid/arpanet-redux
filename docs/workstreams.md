...
- **Decision:** Coordinate the selected application-failover slice with NCC rather than adding another host or IMP. The feasibility gate may propose a new configured IMP 62/IMP 7 edge, but that edge and the existing application edge remain unmapped until exact reciprocal report evidence establishes their line identities. A formal fault claim must separately prove the injected cable state, continued NCC observability, post-cut application success, exact alternate-route H316 observations, and complete cleanup.
