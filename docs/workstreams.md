...
The second was in the external `imp11a-device` source: it copied raw PDP-11 DMA words to a high-bit-first IMP serial interface. Local `432080b2` corrects output, and local `4509a22` applies the symmetric correction to input before guest DMA; local `f1ca562` adds a packet-debug record of the received wire word and the guest word written. They have not been pushed. The research drivers make a run-local `HI2 noconvert` configuration for the PDP-11 attachment by default. Do not change [`pins/sources.lock.toml`](pins/sources.lock.toml) until canonical `origin` publishes a reviewed immutable descendant that includes all three commits.
