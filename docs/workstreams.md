...
- **State:** `main` contains accepted version-1 application and version-2 network-behavior completed-summary profiles, the unchanged bounded version-1 controller stream, deterministic replay and static viewing, genuine Type 301/303/302 report ingestion and event replay, topology-aware paired-line reconciliation with canonical `up`, `down`, and `looped` gates, a read-only final-snapshot adapter for supported fault and loopback results, and separate passive historical-line and message-journey browser displays. The dedicated NCC branch additionally contains the accepted seven-component coexistence composition, its verified completed-result projection, a restrained topology-first network board, the earlier scenario folio retained as `/report`, and a one-command terminal runner over the unchanged formal smoke. The board selects only the existing progressive or completed projection; its mapped line preserves accepted support 322/355 separately from the later receiver tail.
- **Decision:** Integrate the verified coexistence composition, passive network board, detailed report, and terminal-owned runner from the dedicated NCC worktree before selecting another simulation expansion. After integration, choose explicitly between an application-relevant alternate-route fault and a new bounded host/IMP composition; neither is authorized merely because the board exists. Interactive TELNET input, browser controls, per-component restart, a complete KA10 or IMP11-A grammar, and original NCC System 52 remain separate later choices.
...
- **Decision:** The coexistence route, passive network board, and detailed report are complete on the dedicated NCC branch; no additional route expansion is selected before that slice is integrated. Any application-relevant fault route or new host/IMP composition is a separate coordinated NCC/network-expansion gate with its own bounded claim and evidence.
