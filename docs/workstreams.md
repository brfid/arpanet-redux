...
|---|---|---|---|
| Laboratory usability | Lifecycle stabilization and persistent direct PDP-11/ITS workspaces are complete. Both guests preserve created and modified files across repeated restarts; guest shutdown, exact source upload, verified publication, exclusive ownership, interrupted-save recovery, and rollback are evidenced | No follow-up is selected. Use named external workspaces for saved guest files and retain fresh-image acceptance runs. Full memory/session suspend, persistent failover/NCC compositions, automatic stale-lock recovery, and broader failover tracing remain deferred | [Workspace contract](workspaces.md), [accepted result](experiments/2026-09-05-persistent-guest-workspaces.md), [ADR-019](adr/0019-persistent-direct-guest-disks.md), [harness contracts](harness.md), [run diagnostics](runbook.md#diagnose-a-retained-run) |
