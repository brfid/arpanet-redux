...
The historical-line up/down/looped evidence slice is complete. Exact run `ncc-imp6-original-20260831T215714Z` used the unchanged proof configuration and produced checksum-valid Type 303 and Type 302 reports independently attributed to IMP 6, including repeated `imp:6:line:1` up observations toward IMP 5. The shared modem binding explicitly maps IMP 5 line 1 and IMP 6 line 1, and the retained sidecar's final fresh pair reduces to `up` with direct support sequences 103 and 114. Exact clean run `ncc-alternate-path-fault-canonical-20260831T230203Z` then kept both endpoints observable and forced IMP 6 reports through configured test peer IMP 7 while an owned relay dropped the direct cable in both directions; the supported evaluator reduced pre-cut sequences 80 and 91 to `up` and final sequences 377 and 387 to `down`, with nine post-cut trouble reports from each endpoint. Exact clean run `ncc-line-loopback-canonical-20260831T235244Z` used the same alternate route while the owned reflector returned both direct directions to their source; the supported evaluator reduced pre-loop sequences 80 and 90 to `up` and final self-neighbor sequences 397 and 419 to `looped`, with ten post-loop trouble reports from each endpoint and all eight checks passing. The [IMP 6 experiment](experiments/2026-08-31-ncc-imp6-report-proof.md), [alternate-path experiment](experiments/2026-08-31-ncc-alternate-path-fault.md), and [loopback experiment](experiments/2026-08-31-ncc-line-loopback.md) own the evidence; [ADR-006](adr/0006-ncc-line-reconciliation.md), [ADR-009](adr/0009-ncc-paired-line-topology-boundary.md), [ADR-010](adr/0010-ncc-down-report-neighbor-absence.md), and [ADR-011](adr/0011-ncc-looped-report-self-neighbor.md) own the accepted reducer and mapping rules. No durable reducer output or bridge to the accepted run-summary/controller-live contracts was added.

The owned reflector, structured evaluator, bounded lifecycle runner, test-plan contract, runbook entry, and `smoke-ncc-line-loopback` target are implemented and canonically exercised. After feature integration, no further historical-line evidence task is selected. Decide explicitly whether the next NCC work is the independent persisted-output/contract bridge, a message-journey harness consumer, or another bounded product slice; do not infer that choice from this completed gate. Do not relabel the original failed run, synthesize report payloads, infer an alternate report-line identity, or make topology substitute for observed state.
