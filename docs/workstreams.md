...
- **State:** The two-IMP NCC attachment, coordinated three-IMP fault and loopback compositions, seven-component application/NCC coexistence composition, passive network board, and detailed report are integrated on `main`. [`imp5-ncc-host-interface.json`](../config/topologies/imp5-ncc-host-interface.json) owns the evidenced IMP 5/IMP 6 report-line mapping; [`ncc-alternate-path-fault.json`](../config/topologies/ncc-alternate-path-fault.json) adds IMP 7 and two deliberately unmapped alternate bindings; [`ncc-pdp11-its-coexistence.json`](../config/topologies/ncc-pdp11-its-coexistence.json) adds the accepted heterogeneous application route on IMP 6 MI3 without mapping it. The NCC feature branch configures an additional IMP 62 / IMP 7 edge and a cut relay on the application edge, but neither exact attempt reached the cut.
- **Decision:** Do not add another host or IMP and do not integrate the feature branch yet. The application and alternate edges remain unmapped. One clean post-correction run must separately prove guest host readiness, the injected cable state, continued NCC observability, same-session post-cut application success, exact alternate-route H316 observations, and complete cleanup. The two retained pre-cut failures do not show that the alternate route is unsupported; they show only that its runtime acceptance gate has not yet been reached.
