...
|---|---|---|---|
| Repository maintenance | Phases 1 and 2 are complete; RM-07 gives the two-ITS controller ordinary focused owners with a passing real smoke, and characterization now pins the direct PDP-11 controller's remaining reusable lifecycle contracts | Immediate: extract the direct PDP-11/ITS helpers and replace its dynamic two-ITS sibling load with ordinary imports. Following, as a separate smoke-verified migration: remove the failover controller's dynamic direct-controller load | [Repository maintenance plan](repository-maintenance.md), [harness design](harness.md), [test plan](test-plan.md) |
