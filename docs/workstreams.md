...
|---|---|---|---|
| Repository maintenance | Phases 1 and 2 are complete; RM-07 has locked manifest append, PTY interaction and shutdown, IMP logging and liveness, PID attribution, and bounded process cleanup | Finish the first RM-07 slice by moving those process owners and their minimal manifest dependency into an ordinary `ncc` import used by the two-ITS controller, then run its formal smoke before migrating another controller | [Repository maintenance plan](repository-maintenance.md), [harness design](harness.md), [test plan](test-plan.md) |
