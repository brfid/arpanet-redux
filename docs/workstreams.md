...
| PDP-11 TELNET | Gate 4H, receipt-bound media, remote `:TIME`, correlated IMP evidence, ten-observation typed journey, bounded repeated operator commands in one session, a staged terminal boot display, clean option negotiation, and cleanup pass | No required follow-up. Optional work includes character-oriented terminal behavior or remaining bounded NCP anomalies; neither may fill the unproved guest-ingress boundaries by inference | [IMP11-A record](research/imp11a-device.md), [interactive TELNET](experiments/2026-09-01-interactive-pdp11-its-telnet.md), [option negotiation](experiments/2026-09-01-network-unix-telnet-option-negotiation.md) |
