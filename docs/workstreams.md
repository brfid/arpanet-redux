...
|---|---|---|---|
| Laboratory usability | The first lifecycle stabilization batch is complete: direct startup stages and early failures are retained, readiness waits detect owned peer exits, repeated interruption preserves cleanup, partial launches release resources, and cleanup retries relinquish successful releases. Real timeout, peer-exit, repeated-signal, two normal direct cycles, and NCC coexistence checks passed with preserved inputs and released resources; all 404 source tests and Linux/macOS CI passed | No follow-up is selected. Guest media remains disposable. Make prerequisite failures and uncatchable termination can still lack records; other controllers and auxiliary processes retain their existing readiness checks. Persistent workspaces, automatic stale-lock recovery, and broader failover tracing are deferred | [Lifecycle recovery](experiments/2026-09-05-lifecycle-recovery.md), [harness contracts](harness.md), [base reconstruction](experiments/2026-09-04-pdp11-base-reconstruction.md), [run diagnostics](runbook.md#diagnose-a-retained-run) |
