...
The application proof remains open, but the apparent header rejection is now closed. The correlated run `imp11a-telnet-ka10-trace-20260831T131616Z` traces the forwarded request through the KA10 attachment, proves ITS reads it, and observes ITS return the required RRP plus later NCP control traffic; the IMPs emit normal type-5 RFNMs, not type-1 errors. The complete NOSC nine-byte header matches both the January 1972 Host/Host specification and the pinned ITS monitor's NCP-era field layout. The earlier console line is a truncated `kr_rcv: Data from host zero`; its first numeric byte is the daemon's receive opcode, not an IMP type. The remaining blocker is now at the guest input boundary: unchanged numeric IMP words are written into little-endian PDP-11 memory so the byte-structured driver reads the returned `0, 106` type/host pair as `106, 0` and drops valid replies. No compatibility conversion has been added. The next evidence-producing action is to correct that input DMA presentation under the established driver contract and rerun the same correlated transaction, requiring the guest to decode RRP and the subsequent ITS control messages. See [the dated device record](research/imp11a-device.md#ka10-side-trace-its-accepts-the-nosc-header-2026-08-31) for the primary-source comparison and exact-run evidence.
