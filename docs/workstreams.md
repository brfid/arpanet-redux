...
|---|---|---|---|
| Repository maintenance | Phase 1 is complete; RM-05 has characterized the four passive HTTP adapters and their current wire contracts | Finish RM-05 by extracting only neutral transport mechanics while preserving each display application's routes, errors, snapshots, authority, identity, and logging policy | [Repository maintenance plan](repository-maintenance.md), [NCC observability](ncc.md), [test plan](test-plan.md) |
