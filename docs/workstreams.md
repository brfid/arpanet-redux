...
| `/Users/brf/src/arpanet-redux-worktrees/persistent-workspaces` | `codex/persistent-workspaces` | Persistent direct PDP-11/ITS guest workspaces |
...
|---|---|---|---|
| Laboratory usability | Lifecycle stabilization is complete. Persistent workspaces are selected for the direct PDP-11/ITS composition; implementation begins by proving guest-safe shutdown and reboot of both disk sets | Preserve saved guest files across complete process stop/restart using named external workspaces, exclusive ownership, verified generation publication, and retained previous saves. First prove file creation and modification on both guests, repeated restarts and TELNET, and interrupted-save recovery. Fresh-image acceptance runs retain their current behavior. Full memory/session suspend, failover workspaces, automatic stale-lock recovery, and broader failover tracing remain deferred | [Lifecycle recovery](experiments/2026-09-05-lifecycle-recovery.md), [harness contracts](harness.md), [base reconstruction](experiments/2026-09-04-pdp11-base-reconstruction.md), [run diagnostics](runbook.md#diagnose-a-retained-run) |
