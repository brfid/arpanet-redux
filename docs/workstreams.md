...
|---|---|---|---|
| Repository maintenance | Phases 1 and 2 are complete; RM-07 gives the two-ITS, direct PDP-11, and failover controllers ordinary focused harness owners, each with a passing real smoke | Immediate: migrate the interactive controller, remove temporary compatibility exports, and guard against production sibling-script loading. Following: run the deterministic interactive smoke and close RM-07 if all acceptance criteria hold | [Repository maintenance plan](repository-maintenance.md), [harness design](harness.md), [test plan](test-plan.md) |
