...
|---|---|---|---|
| Laboratory usability | Two batches complete: `make diagnose-run RESULT=...` explains retained runs, including new launcher failure, handled-signal, and cleanup records. Exact interruption exposed a five-second controller shutdown limit that left simulators alive; repair `97e3561` gives controllers time to complete and keeps forced controller termination a cleanup failure. All 379 source tests passed with one expected UDP sandbox skip; runtime checks passed with their expected socket skip. Fresh startup-failure, repaired-interruption, direct TELNET, and NCC loopback checks passed, and five older results remained unchanged | No follow-up is selected. Independent base-media reconstruction remains the next adoption priority. Failures before result creation and uncatchable termination can still lack records; missing evidence remains unknown. Historical protocols, guest media, browser authority, and acceptance rules were unchanged by these batches | [Cleanup evidence](experiments/2026-09-04-launcher-cleanup.md), [harness](harness.md#launcher-termination-records), [run diagnostics](runbook.md#diagnose-a-retained-run) |
