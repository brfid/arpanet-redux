...
| `/Users/brf/src/arpanet-redux-worktrees/run-diagnostics` | `codex/run-diagnostics` | Completed first batch; GitHub publication awaits approval |
...
|---|---|---|---|
| Laboratory usability | First batch complete locally at `5821577`: `make diagnose-run RESULT=...` explains retained successful, failed, and unfinished runs, keeping controller and runtime outcomes and cleanup layers distinct. All 359 source tests completed with one expected UDP sandbox skip; runtime checks passed with their expected socket skip. Five retained direct, interactive-failover, failed two-ITS, line-fault, and loopback results produced deterministic reports without changing their files | Automatic approval review rejected publication to GitHub `main`; the tested commits are integrated locally and await explicit push approval. No next batch is selected. Prioritize independent base-media reconstruction and explicit failure/cleanup recording in paths that currently retain no explanation. Missing records remain unknown; recorded outcomes are not acceptance revalidation or current process liveness | [Harness](harness.md#retained-run-diagnostics), [run diagnostics](runbook.md#diagnose-a-retained-run), [NCC authority](ncc.md) |
