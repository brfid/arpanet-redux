...
The topology dependency and reciprocal report-line fields are consumed and checked by the NCC code. The fields record only the exact project proof mapping; they do not assert that every simulator `MI1` is historical report line 1 or turn configured topology into observed network evidence. `make smoke-ncc-alternate-path` owns the accepted fault lifecycle and external evidence; [the dated experiment](experiments/2026-08-31-ncc-alternate-path-fault.md) records its canonical run and limits. The [loopback experiment](experiments/2026-08-31-ncc-line-loopback.md) records external-laboratory evidence only until its self-neighbor reducer rule and owned harness are separately accepted and promoted. Any further route expansion remains a coordinated NCC/network-expansion change under the evidence boundary recorded by [ADR-009](adr/0009-ncc-paired-line-topology-boundary.md) and [ADR-010](adr/0010-ncc-down-report-neighbor-absence.md).
