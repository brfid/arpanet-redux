...
|---|---|---|---|
| Repository maintenance | Phases 1 and 2 are complete; RM-07's first slice gives PTY and IMP process ownership plus validated manifest append ordinary `ncc` owners, with the two-ITS controller migrated and its dependent aliases preserved | Next characterize and extract the remaining manifest-hash and generic log-readiness primitives, then remove the direct PDP-11 controller's dynamic sibling load in its own smoke-verified migration | [Repository maintenance plan](repository-maintenance.md), [harness design](harness.md), [test plan](test-plan.md) |
