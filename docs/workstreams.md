...
|---|---|---|---|
| Repository maintenance | Phase 1 and RM-05 are complete: the four passive displays share neutral HTTP mechanics while retaining application-owned routes, snapshots, errors, identities, and authority | RM-06 is selected; characterize the alternate-path fault and loopback lifecycle, then share only launch, readiness, evidence-capture, and cleanup mechanics while retaining distinct instruments, evaluators, artifacts, and verdicts | [Repository maintenance plan](repository-maintenance.md), [NCC observability](ncc.md), [test plan](test-plan.md) |
