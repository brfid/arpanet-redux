...
| `/Users/brf/src/arpanet-redux-worktrees/ka10-ingress` | `codex/ka10-host-ingress` | Bounded KA10 receive-assembly evidence for Gate 4H request ingress |
...
| KA10 host ingress | Existing traces cannot bind a consumed `DATAI` value to its receive-assembly width, valid-bit count, message identity, and word index, so direct Gate 4H `boundary:request:6` remains unproved | Add observation-only receive-assembly evidence in a separate simulator worktree and run one exact direct Gate 4H transaction. Add a parser and typed observation only if that evidence independently reconstructs the accepted request; do not change a guest, protocol behavior, application gate, reply ingress, or failover claim | [ADR-013](adr/0013-ncc-message-journey-stream.md), [KA10 feasibility result](experiments/2026-09-01-ka10-host-ingress-grammar.md) |
