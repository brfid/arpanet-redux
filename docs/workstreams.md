...
|---|---|---|---|
| Repository maintenance | Phases 1 and 2 are complete; RM-07 has importable PTY, IMP-process, and manifest-append owners, and characterization now pins the remaining two-ITS hashing, readiness, environment, and cleanup contracts | Immediate: extract those characterized primitives into focused ordinary `ncc` modules. Following, as a separate smoke-verified migration: remove the direct PDP-11 controller's dynamic sibling load | [Repository maintenance plan](repository-maintenance.md), [harness design](harness.md), [test plan](test-plan.md) |
