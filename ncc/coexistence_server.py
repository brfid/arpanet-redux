...
class CoexistenceDisplayResponse:
...
def create_coexistence_display_server(
    ...
    *,
    ...
    server = CoexistenceDisplayHTTPServer(("127.0.0.1", port), _DisplayHandler)
...
def coexistence_display_response(
    ...
    if method not in {"GET", "HEAD"}:
        return CoexistenceDisplayResponse(
            status=405,
...
class _DisplayHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: object) -> None:
        """Keep the one-shot completed desk quiet in the operator terminal."""

        return

    def _dispatch(self, method: str) -> None:
        response = coexistence_display_response(
            self.server.display,
            self.server.page,
            method,
            self.path,
