"""Decode the fixed-format Type 301 IMP trouble report.
...
@dataclass(frozen=True)
class TroubleReport:
    """Decoded semantic fields from a Type 301 report."""
...
def decode_trouble_report(raw_words: Iterable[int]) -> TroubleReport:
    ...
    if len(words) not in (SEMANTIC_WORD_COUNT, PADDED_WORD_COUNT):
        raise ValueError(
            "Type 301 trouble reports require 31 semantic words "
    ...
    if words[0] != TROUBLE_REPORT_TYPE:
        raise ValueError(
            f"expected Type 301 ({TROUBLE_REPORT_TYPE:#o}), got {words[0]:#o}"
        )
